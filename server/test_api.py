...
def test_post_transactions():
    ...
    for txn in SAMPLE_TRANSACTIONS:
        ...
        if r.status_code in (200, 201):
            data = r.json()
            print(f"   ✓ Posted '{txn['type']} {txn['currency']} {txn['amount']}' → id={data.get('id')}")
...
def test_duplicate():
    ...
    if r.status_code == 400:
        print("   ✓ Duplicate correctly rejected (HTTP 400)")
    else:
...
def test_filter_type():
    ...
    if r.status_code == 200:
        ...
        print(f"   ✓ Got {len(data)} 'Received' transactions — all correct: {all_received}")
    else:
        print(f"   ✗ Failed: HTTP {r.status_code}")
...
def test_search():
    ...
    if r.status_code == 200:
        data = r.json()
        print(f"   ✓ Search returned {len(data)} result(s): {[t['sender'] for t in data]}")
    else:
        print(f"   ✗ Failed: HTTP {r.status_code}")
...
def test_sort_amount():
    ...
    if r.status_code == 200:
        ...
        print(f"   ✓ Amounts in order: {amounts} — sorted: {is_sorted}")
    else:
        print(f"   ✗ Failed: HTTP {r.status_code}")
...
if __name__ == "__main__":
    ...
    if not test_health():
        print("\n  ✗ Server is not running. Start it first and retry.")
        sys.exit(1)
